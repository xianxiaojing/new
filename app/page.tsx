...
const projects = [
  {id:"01",title:"核心借款链路体验升级",subtitle:"用流程重构推动信贷转化",metric:"11.3% → 22.14%",tag:"信贷转化 · 2022",image:"/works/loan-flow.png",brief:"将输入金额、选择期数、确认借款三步合并为一步决策；金额默认展示系统最高可借额度，期数变化时利率与还款信息实时反馈。",role:"交互流程优化、数据建设",result:"同口径下，提交借款转化率由 11.3% 提升至 22.14%。"},
  {id:"02",title:"拒量导流授信流程优化",subtitle:"承接被拒用户的二次价值",metric:"累计 +7.11pp",tag:"授信承接 · 2024",image:"/works/credit-routing.png",brief:"在明确第三方服务主体与个人信息授权的前提下，复用已有资料，仅展示仍需补充的信息，并重构为两组连续表单。",role:"交互流程优化",result:"1.0 表单优化提升 4.46pp；2.0 新增补充授信入口再提升 2.65pp。"},
  {id:"03",title:"桔耀会员商业化增长",subtitle:"独立权益场景的体验与增长",metric:"M1→M2 续费 70%+",tag:"会员商业化 · 2025—2026",image:"/works/membership.png",brief:"从售前营销策略、需求分析到会员落地页与权益楼层设计，以清晰价值表达提升屏幕效率与购买判断。",role:"业务方案、界面交互、数据复盘",result:"单月营收超目标；续费率 70%+，平均当日退款比例约 2%，M2 续费订单约 1%。"}
];
...
export default function Home(){
  ...
  return <main style={{"--progress":scroll} as React.CSSProperties}>
    ...
    <header><a className="brand" href="#top">ZXJ<span>®</span></a><nav><a href="#about">关于</a><a href="#experience">经历</a><a href="#works">项目</a></nav><a className="contact" href="tel:13051328631">联系我 ↗</a></header>
    ...
    <section id="experience" className="panel"><div className="section-no">02 / EXPERIENCE</div><h2>在真实业务里，<br/>持续验证设计价值。</h2><div className="timeline"><article><b>2022</b><h3>核心借款转化</h3><p>重构三步借款流程，补齐关键行为与服务端埋点。</p></article><article><b>2024</b><h3>拒量授信承接</h3><p>两轮表单与入口迭代，改善被拒用户二次申请体验。</p></article><article><b>2025—2026</b><h3>会员商业化</h3><p>参与策略、方案、设计与复盘，搭建独立会员权益场景。</p></article></div></section>
    ...
    {active&&<div className="modal" onClick={()=>setActive(null)}><article onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setActive(null)}>关闭 ×</button><small>{active.tag}</small><h2>{active.title}</h2><Image src={active.image} alt={active.title} width={1200} height={700}/><div className="detail"><p><b>设计动作</b>{active.brief}</p><p><b>我的角色</b>{active.role}</p><p><b>业务结果</b>{active.result}</p></div></article></div>}
  </main>
}
